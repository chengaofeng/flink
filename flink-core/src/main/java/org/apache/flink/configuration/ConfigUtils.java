...
public class ConfigUtils {
	...
	/**
	 * Puts an array of values of type {@code IN} in a {@link Configuration}
	...
	/**
	 * Puts a {@link Stream} of values of type {@code IN} in a {@link Configuration}
	 * as a {@link ConfigOption} of type {@link List} of type {@code OUT}.
	 *
	 * @param configuration the configuration object to put the list in
	 * @param key the {@link ConfigOption option} to serve as the key for the list in the configuration
	...
	public static <IN, OUT> void encodeStreamToConfig(
		...
		final List<OUT> encodedOption = values.map(mapper).filter(Objects::nonNull).collect(Collectors.toList());
	...
	/**
	 * Gets a {@link List} of values of type {@code IN} from a {@link Configuration}
